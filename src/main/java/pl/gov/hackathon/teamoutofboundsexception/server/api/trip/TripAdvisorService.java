...
public class TripAdvisorService {
    ...
    public List<List<TripPlaceDTO>> getTripAdvise(TripPlacePromise promise) {
        ...
        List<PlaceModel> tempList = placeRepository.findByCityNameAndMapXBetweenAndMapYBetweenAndOpeningTimeIsBeforeAndClosingTimeIsAfterAndImageUrlIsNull(promise.getActualPlaceName(), promise.getLongitude() - (float) 20.0, promise.getLongitude() + (float) 20.0,promise.getLatitude() - (float) 20.0, promise.getLatitude() + (float) 20.0, x, x);
